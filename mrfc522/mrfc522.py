...
class MIFAREReader:
    ...
    @staticmethod
    def read_write_card(timeout, auth_callback=None, write_callback=None):
        # Welcome message
        mfr = MIFAREReader()
	...
	if not timeout or timeout < 1:
        ...
        while time.time() <= start_time + timeout:
            ...
            (status,uid) = mfr.MFRC522_Anticoll()
            ...
            if status == mfr.MI_OK:
                ...
                mfr.MFRC522_SelectTag(uid)

                # see if we know this uid
                key = auth_callback(uid) or mfr.DEFAULT_KEY

                # Authenticate
                status = mfr.MFRC522_Auth(mfr.PICC_AUTHENT1A, 8, key, uid)

                # Check if authenticated
                if status != mfr.MI_OK:
                    mfr.MFRC522_StopCrypto1()
                    return [uid, None]
                mfr.MFRC522_Read(8)
                if write_callback:
                    data = write_callback(uid)
                    # Write the data
                    mfr.MFRC522_Write(8, data)


                    # Check to see if it was written
                    new_key = mfr.MFRC522_Read(8)
                    mfr.MFRC522_StopCrypto1()
                    if new_key == data:
                        return [uid, new_key]
